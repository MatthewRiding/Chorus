...
class ChorusMainWindow(QMainWindow, Ui_MainWindow):
    ...
    # Define signals that can be emitted by the main window:
    # When a new full matrix data set is loaded, a signal will be emitted carrying n_elements, t_min_us and t_max_us
    # in a tuple:
    signal_full_matrix_changed = Signal(tuple)
    # The iso_time_changed signal will need to transmit the new selected time in microseconds as a float.
    signal_iso_t_time_changed = Signal(float)
    # The iso-gen and iso-det index sliders will each emit their own signals, passing their new index as an int:
    signal_iso_gen_index_changed = Signal(int)
    signal_iso_det_index_changed = Signal(int)
    # The delay_matrix_changed signal will need to transmit the new selected delay matrix (in s) as a 2D numpy ndarray.
    signal_delay_matrix_changed = Signal(np.ndarray)
    ...
    def __init__(self):
        ...
        # Set window size to fill 80% of screen:
        ...
        # Action to toggle the data volume viewer window:
        self.action_toggle_data_volume_widget = QAction('Data volume viewer', parent=self)
        self.action_toggle_data_volume_widget.setCheckable(True)
        self.action_toggle_data_volume_widget.triggered.connect(self.toggle_data_volume_widget)
        ...
        self.menu_windows.addActions([self.action_toggle_pcm_viewer, self.action_toggle_data_volume_widget])
        ...
        self.slider_time_index.valueChanged.connect(self.iso_t_time_index_changed_by_slider)
        self.doubleSpinBox_time_us.valueChanged.connect(self.iso_t_time_changed_by_spinbox)
    ...
    def det_index_changed(self, index):
        ...
        self.iso_time_plot_widget.update_det_line(self.index_iso_det)
        self.iso_time_plot_widget.blit_manager.blit_all_animated_artists()
        # Emit iso-det index changed signal for any connected widgets:
        self.signal_iso_det_index_changed.emit(index)
    ...
    def gen_index_changed(self, index):
        ...
        self.iso_time_plot_widget.update_gen_line(self.index_iso_gen)
        self.iso_time_plot_widget.blit_manager.blit_all_animated_artists()
        # Emit iso-gen index changed signal for any connected widgets:
        self.signal_iso_gen_index_changed.emit(index)

    def iso_t_time_index_changed_by_slider(self, index):
        ...
        self.iso_t_time_value_changed(time_us)

    def iso_t_time_changed_by_spinbox(self, iso_t_time_us):
        ...
        index = int((iso_t_time_us - self.full_matrix.t_min_us) // self.full_matrix.period_sampling_us)
        ...
        self.time_index = index
        # Call the generic time value changed method:
        self.iso_t_time_value_changed(iso_t_time_us)
    ...
    def iso_t_time_value_changed(self, time_us):
        self.time_iso_t_us = time_us
        ...
        # Update the isochron:
        self.isochron_manager.selected_time_changed(self.time_iso_t_us * 10 ** -6)
    ...
    def macro_new_fmclp_dataset(self):
        ...
        # Emit the full matrix changed signal for any connected widgets:
        self.signal_full_matrix_changed.emit((self.full_matrix.n_elements,
                                              self.full_matrix.t_min_us, self.full_matrix.t_max_us))
        # Emit the current time and gen & det indices to reset data in other widgets:
        self.signal_iso_t_time_changed.emit(self.time_iso_t_us)
        self.signal_iso_gen_index_changed.emit(self.index_iso_gen)
        self.signal_iso_det_index_changed.emit(self.index_iso_det)
    ...
    def update_iso_det_plot(self):
        ...
        if self.selected_tfm_image and self.selected_tfm_image.complete and self.pixel_selected:
            delay_vector_iso_det = self.delay_matrix_s[self.index_iso_det, :]
    ...
    def update_iso_gen_plot(self):
        ...
        # If a pixel is selected, update the delays shown:
        if self.selected_tfm_image and self.selected_tfm_image.complete and self.pixel_selected:
            delay_vector_iso_gen = self.delay_matrix_s[:, self.index_iso_gen]
    ...
    def complete_tfm_image_selected(self):
        ...
        if self.pixel_selected:
            ...
            self.clear_delay_visualisations()
    ...
    def clear_delay_visualisations(self):
        # Clear delay times & hide delay markers on all raw data plots:
        ...
        # Emit delay matrix changed signal with an empty array:
        self.signal_delay_matrix_changed.emit(np.array([]))
    ...
    def tfm_pixel_clicked(self, xz_coords_m_tuple):
        ...
        if xz_coords_m_tuple[0] is None:
            # Clear the delay data from all relevant widgets:
            self.clear_delay_visualisations()
        else:
            ...
            if self.selected_tfm_image:
                if self.selected_tfm_image.complete:
                    ...
                    # Generate the delay matrix for this pixel and TFM image:
                    self.delay_matrix_s = generate_delay_matrix(x_pixel_m, z_pixel_m, self.selected_tfm_image)

                    # Display the delays on the B-scan widgets:
                    self.b_scan_view_widget_iso_det.update_delays(self.delay_matrix_s[self.index_iso_det, :])
                    self.b_scan_view_widget_iso_det.make_delays_visible()
                    self.b_scan_view_widget_iso_det.blit_manager.blit_all_animated_artists()

                    self.b_scan_view_widget_iso_gen.update_delays(self.delay_matrix_s[:, self.index_iso_gen])
                    ...
                    # Emit the delay matrix changed signal for any connected widgets:
                    self.signal_delay_matrix_changed.emit(self.delay_matrix_s)
    ...
    def toggle_data_volume_widget(self):
        if self.data_volume_widget:
            # The widget already exists.  Close it:
            self.data_volume_widget.close()
            # Set Python reference to None:
            self.data_volume_widget = None
        else:
            # The widget is not already open.  Create a new instance & show it:
            self.data_volume_widget = DataVolumeVisWidget(self.signal_iso_t_time_changed,
                                                          self.signal_iso_gen_index_changed,
                                                          self.signal_iso_det_index_changed,
                                                          self.signal_delay_matrix_changed,
                                                          self.signal_full_matrix_changed,
                                                          self.full_matrix.n_elements, self.full_matrix.t_min_us,
                                                          self.full_matrix.t_max_us, self.time_iso_t_us,
                                                          self.index_iso_gen, self.index_iso_det, self.delay_matrix_s)
            # Wire signals to slots:
            self.data_volume_widget.signal_closed.connect(self.slot_data_volume_widget_closed)
            # Open the window:
            self.data_volume_widget.show()

    def slot_data_volume_widget_closed(self):
        self.action_toggle_data_volume_widget.setChecked(False)
        # Set Python reference to None:
        self.data_volume_widget = None
